    ...
    <div class="flex-none w-full max-w-full px-3">
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-lg rounded-lg">
        ...
        <div class="flex p-4 pt-1 overflow-x-auto whitespace-no-wrap">
          @php
            $statuses = ['all' => 'All', 'processing' => 'Processing', 'confirmed' => 'Confirmed', 'shipped' => 'Shipped', 'delivered' => 'Delivered', 'cancelled' => 'Cancelled'];
          @endphp
          ...
          @foreach ($statuses as $key => $label)
            ...
            <a href="{{ $key === 'all' ? route('customer.orders') : route('customer.orders', ['status' => $key]) }}" class="flex h-8 items-center justify-center px-4 rounded-full text-sm font-medium mr-2 
            ...
            @foreach ($orders as $order)
              <div class="flex items-start justify-between mb-4">
                <div>
                  ...
                  <a href="{{ route('customer.details', ['user_id' => optional($order->user)->id ?? $order->user_id]) }}">
