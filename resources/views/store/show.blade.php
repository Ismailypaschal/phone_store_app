...
<div class="bg-white rounded-lg shadow-sm overflow-hidden max-w-3xl">
    <img src="{{ $product['image'] }}" alt="{{ $product['brand'] }} {{ $product['model'] }}" class="w-full h-64 object-cover">
    <div class="p-6">
        <h1 class="text-xl font-semibold">{{ $product['brand'] }} {{ $product['model'] }}</h1>
        <p class="text-gray-500">{{ $product['storage_gb'] }} GB • {{ $product['color'] }}</p>
